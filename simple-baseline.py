...
# feature 3: tf-idf
def get_feature_tfIdf(X_train):
    print("generating tf-idf features")
    cv = CountVectorizer()
    cv_res = cv.fit_transform(X_train)
    tf_idf = TfidfTransformer()
    tf_idf_res = tf_idf.fit_transform(cv_res)
    pca = TruncatedSVD(50)
    ans = pca.fit_transform(tf_idf_res)
    return ans
...
def swn_score(text):
    ...
    if tokens_count == 0:
        output = np.array([0,positive_count,negative_count,positive_score,negative_score,ratio], dtype = 'float32')
#             return np.array([0,positive_count,negative_count,positive_score,negative_score,ratio], dtype = 'float32')
    else:
        output = np.array([sentiment,positive_count,negative_count,positive_score,negative_score,ratio], dtype = 'float32')
#             return np.array([sentiment,positive_count,negative_count,positive_score,negative_score,ratio], dtype = 'float32')
    if output is None:
        print("tokens count: ", tokens_count)
        if tokens_count != 0:
            print("sentiment: ", sentiment)
        print("output: ", [positive_count,negative_count,positive_score,negative_score,ratio])
    return output
...
def get_features(data):
    ...
    feature_3 = get_feature_tfIdf(data)
    feature_4 = extract_senti_based_features(data)
    X = np.hstack((feature_1,feature_2,feature_3,feature_4))
