...
def main(args):
    ...
    for i in range(15):
        ...
        km=KMeans(5,'random',n_init=30,max_iter=300,random_state=0)
        km.fit(colective_features[i])
        y_pred = km.predict(X_test_lst[i])
        acc_lst.append(compute_accuracy(y_test, y_pred))
        print(acc_lst)'''

    '''import matplotlib.pyplot as plt
    pca_distortions=[]
    for i in range(1,11):
        km=KMeans(i,'random',n_init=30,max_iter=300,random_state=0)
        km.fit(colective_features[9])
    ...
    pca_distortions=[]
    for i in range(3,11):
        gmm = GaussianMixture(n_components=i, covariance_type='full', random_state=97)
        cluster2 = gmm.fit(colective_features[9]).predict(X_test_lst[9])
        
        pca_distortions.append(silhouette_score(X_test, cluster2))
        
    plt.plot(range(2,11),pca_distortions,marker='o')
    plt.xlabel('Number of clusters')
    plt.ylabel('Distortion')
    plt.show()'''
